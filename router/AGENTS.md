...
Locks: `cache.rs` and `embed.rs` hold one each, and they are independent — there is no
acquisition order between them, and `cache.rs` documents one acquisition per method.
