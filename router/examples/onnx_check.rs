...
fn main() {
    ...
    let dir =
        std::env::var("WATTROUTER_MODEL_CACHE").unwrap_or_else(|_| "/tmp/wattrouter-models".into());
    ...
    for (name, e) in [
        ("hash", &HashEmbedder::new() as &dyn Embedder),
        ("onnx", &onnx),
    ] {
        ...
        println!(
            "{name:5}  hard~easy {s:.3}   hard~alsohard {d:.3}   gap {:+.3}",
            d - s
        );
    }
    ...
    let t = std::time::Instant::now();
    for _ in 0..50 {
        onnx.embed(hard).unwrap();
    }
    println!(
        "onnx embed: {:.1} ms/call",
        t.elapsed().as_secs_f64() * 1000.0 / 50.0
    );
}
#[cfg(not(feature = "onnx"))]
fn main() {
    println!("built without the onnx feature");
}
