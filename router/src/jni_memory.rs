...
//!
//! `handed` is `jni_answer.rs`'s. The copy that used to be at the bottom of
//! this file was identical to `jni_git.rs`'s and neither applied the rules
//! `jni.rs` states; see #468 and #482. The panic guard is no longer either
//! file's: `with_env` catches, and its policy answers with the default.
...
pub extern "system" fn Java_com_getlora_wattrouter_Memory_nativeOpen<'a>(
    ...
    unowned
        .with_env(|env| -> jni::errors::Result<jlong> {
            ...
            // see it happen.
            Ok(crate::core_memory::open(Path::new(&path), keep)
...
pub extern "system" fn Java_com_getlora_wattrouter_Memory_nativeRemember<'a>(
    ...
    unowned
        .with_env(|env| -> jni::errors::Result<jstring> {
            // The null-handle check is here now. It was `memory.as_ref()`
            // inside `core_memory`, which is where it had to be while that
            // function took a pointer. The pointer stops at this file.
            ...
            Ok(handed(env, memory.remember(&session, &speaker, &text, ts)))
        })
        .resolve::<LogErrorAndDefault>()
}
