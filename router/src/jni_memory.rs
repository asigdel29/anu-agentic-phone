...
pub extern "system" fn Java_com_getlora_wattrouter_Memory_nativeFree(
    ...
    unowned
        .with_env(|_env| -> jni::errors::Result<()> {
            if handle != 0 {
                // The other half of `nativeOpen`'s `Box`. `Memory` has no `Drop`
                // of its own: the SQLite handle inside it is closed because the
                // box is dropped, so this has to stay a drop and must not become
                // a forget.
...
pub extern "system" fn Java_com_getlora_wattrouter_Memory_nativeRemember<'a>(
    ...
    unowned
        .with_env(|env| -> jni::errors::Result<jstring> {
            ...
            // `core_memory`, which is where it had to be while that function
            // took a pointer. The pointer stops at this file.
