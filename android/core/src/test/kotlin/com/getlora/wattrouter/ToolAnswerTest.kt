...
//
// The case to read first is the default. Seventeen tools answer prose and none
// of them was edited, because `answer` wraps `run` unless a tool says
// otherwise. A change that made all seventeen say `Answer("…")` would be a
...
class ToolAnswerTest {
    ...
    fun aToolThatOnlyAnswersProseNeedsNoChanges() {
        // The default, which is the whole reason seventeen files are untouched.
        runTest {
            val says = Says("done")
