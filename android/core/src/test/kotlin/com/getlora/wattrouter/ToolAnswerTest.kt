...
//
// The case to read first is the default. Eighteen tools answer prose and none
// of them was edited, because `answer` wraps `run` unless a tool says
// otherwise. A change that made all eighteen say `Answer("…")` would be a
...
class ToolAnswerTest {
    ...
    fun aToolThatOnlyAnswersProseNeedsNoChanges() {
        // The default, which is the whole reason eighteen files are untouched.
        runTest {
            val says = Says("done")
